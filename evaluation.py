...
def evaluation(model, data_loader, device, args):
    with torch.no_grad():
        ...
        for i in tqdm(range(0, num_text, text_bs)):
            ...
            text_embed = model.encode_text(text_input).cpu()
        ...
        for image, image_neg, index in tqdm(data_loader):
            ...
            image_embed = model.encode_image(image).cpu()
            image_embeds.append(image_embed)

            image_neg = image_neg.to(device)
            image_neg_embed = model.encode_image(image_neg).cpu()
        ...
        count_text = (sims_neg <= sims).sum().item()
        count_text = count_text / len(sims) * 100
        print("txt neg acc:", count_text)
        ...
        count_image = (sims_neg <= sims).sum().item()
        count_image = count_image / len(sims) * 100
        print("img neg acc:", count_image)

        score_matrix_i2t = score_matrix_i2t
        score_matrix_t2i = score_matrix_t2i
        score_matrix_i2i = score_matrix_i2i
        score_matrix_t2t = score_matrix_t2t
        total_time = time.time() - start_time
        total_time_str = str(datetime.timedelta(seconds=int(total_time)))
        print('Evaluation time {}'.format(total_time_str))

        return score_matrix_i2t, score_matrix_t2i, score_matrix_i2i, score_matrix_t2t, count_text, count_image
...
def itm_eval(config, scores_i2t, scores_t2i, scores_i2i, scores_t2t, data_loader, test_sl_rate=1.0, tau_i2t=1.0, tau_t2i=1.0, tau_i2i=1.0, tau_t2t=1.0, device='cuda', flag=False):
# def itm_eval(config, scores_i2t_, scores_t2i_, scores_i2i_, scores_t2t_, data_loader, test_sl_rate=1.0, tau_i2t=1.0, tau_t2i=1.0, tau_i2i=1.0, tau_t2t=1.0, device='cuda', flag=False):
    with torch.no_grad():
        ...
        if flag:
            ...
            for i in range(0, scores_i2t.shape[0], batch_size):
                ...
                arg = torch.topk(a, k=top_k, dim=1)[1]
        ...
        if not flag:
            scores_i2t_sc = scores_i2t
        sorted_values, sorted_indices = torch.sort(scores_i2t_sc, dim=1, descending=True)
        ranks = [torch.where(torch.isin(sorted_indices[i], torch.tensor(data_loader.dataset.i2t_label[i], device=sorted_indices[i].device)))[0][0] for i in range(len(sorted_indices))]

        tr1 = 100.0 * len([x for x in ranks if x < 1]) / len(ranks)
        tr5 = 100.0 * len([x for x in ranks if x < 5]) / len(ranks)
        tr10 = 100.0 * len([x for x in ranks if x < 10]) / len(ranks)

        if not flag:
            scores_t2i_sc = scores_t2i
        sorted_values, sorted_indices = torch.sort(scores_t2i_sc, dim=1, descending=True)
        ranks = [torch.where(torch.isin(sorted_indices[i], torch.tensor(data_loader.dataset.t2i_label[i], device=sorted_indices[i].device)))[0][0] for i in range(len(sorted_indices))]

        ir1 = 100.0 * len([x for x in ranks if x < 1]) / len(ranks)
        ir5 = 100.0 * len([x for x in ranks if x < 5]) / len(ranks)
        ir10 = 100.0 * len([x for x in ranks if x < 10]) / len(ranks)
