...
from X2VLM.scheduler import create_scheduler
from X2VLM.optim import create_optimizer
...
@torch.no_grad()
def evaluation(model, data_loader, tokenizer, device, config, k=64):
    model.eval()
    ...
    texts_neg = data_loader.dataset.text_neg   
    num_text = len(texts)
    text_bs = 128  # 256
    text_feats_neg = []  # in fact, text_embeds
    text_embeds_neg = []  # in fact, text_feats...
    text_atts_neg = []
    for i in range(0, num_text, text_bs):
        text = texts_neg[i: min(num_text, i + text_bs)]
        text_input = tokenizer(text, padding='max_length', truncation=True, max_length=config['max_tokens'],
                               return_tensors="pt").to(device)
        text_feat = model.get_text_embeds(text_input.input_ids, text_input.attention_mask)
        text_embed = model.get_features(text_embeds=text_feat)
        text_feats_neg.append(text_feat)
        text_atts_neg.append(text_input.attention_mask)
        text_embeds_neg.append(text_embed)
    text_embeds_neg = torch.cat(text_embeds_neg, dim=0)
    text_atts_neg = torch.cat(text_atts_neg, dim=0)
    text_feats_neg = torch.cat(text_feats_neg, dim=0)

    image_feats = []
    image_embeds = []
    image_feats_neg = []
    image_embeds_neg = []
    for image, image_neg, index in data_loader:
        ...
        image = image_neg.to(device)
        image_feat, _ = model.get_vision_embeds(image)
        image_embed = model.get_features(image_embeds=image_feat)
        image_feats_neg.append(image_feat)
        image_embeds_neg.append(image_embed)

    image_feats = torch.cat(image_feats, dim=0)
    image_embeds = torch.cat(image_embeds, dim=0)
    image_feats_neg = torch.cat(image_feats_neg, dim=0)
    image_embeds_neg = torch.cat(image_embeds_neg, dim=0)
    ...
    sims_neg = get_similarity(text_embeds, image_embeds_neg, data_loader.dataset.t2i_label)
    ...
    return image_embeds @ text_embeds.t(), text_embeds @ image_embeds.t(), image_embeds @ image_embeds.t(), text_embeds @ text_embeds.t(), count_text, count_image
