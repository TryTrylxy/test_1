...
def main(args, config):
    ...
    if args.eval:
        ...
        # print(score_val_i2t)
        print(score_test_i2t)
        # print(score_val_i2t.shape)
        print(score_test_i2t.shape)
        raise
        def do_45(x):
            reserveNumber = 2
            for key in x.keys():
    ...
    for epoch in range(start_epoch, max_epoch):
        if epoch >= 10 or (epoch - best_epoch) >= 4:
            break
        copy_file("./train.log", config['logger_name'])
        ...
        best_result, best, best_epoch = dev(model, val_loader, test_loader, device, tokenizer, train_stats, epoch, best_result, best, best_epoch)

        peak_mem_gb = get_gpu_peak_memory(unit="GB")
        print(f"===== Epoch {epoch} 显存使用峰值：{peak_mem_gb:.2f} GB =====")
    ...
    if utils.is_main_process():
        ...
        if config['do_abcmargin_ab']:
            with open(os.path.join(config['logger_name'], "abcmargin_ab.txt"), "a", encoding="utf-8") as f:
                f.write(f"abc_margin_alpha:{config['abc_margin_alpha']}\tbest_epoch:{best_epoch}\nresult:{best_result}\n")
        if config['do_tripletlossrate_ab']:
            with open(os.path.join(config['logger_name'], "tripletlossrate_ab.txt"), "a", encoding="utf-8") as f:
                f.write(f"tripletlossrate:{config['tripletlossrate']}\tbest_epoch:{best_epoch}\nresult:{best_result}\n")
    copy_file("./train.log", config['logger_name'])

def dev(model, val_loader, test_loader, device, tokenizer, train_stats, epoch, best_result, best, best_epoch):
    with torch.no_grad():
        # eval
        if config['backbone'] == 'CLIP':
            score_val_i2t, score_val_t2i, score_val_i2i, score_val_t2t, count_text_val, count_image_val = evaluation(model, val_loader, device, args)
            score_test_i2t, score_test_t2i, score_test_i2i, score_test_t2t, count_text_test, count_image_test = evaluation(model, test_loader, device, args)
        elif config['backbone'] == 'X2VLM':
            from X2VLM.Retrieval import evaluation as tmp_ev
            score_val_i2t, score_val_t2i, score_val_i2i, score_val_t2t, count_text_val, count_image_val = tmp_ev(model.x2_model, val_loader, tokenizer, device, model.c)
            score_test_i2t, score_test_t2i, score_test_i2i, score_test_t2t, count_text_test, count_image_test = tmp_ev(model.x2_model, test_loader, tokenizer, device, model.c)
        ...
        if utils.is_main_process():
            def do_45(x):
                ...
                return x
            print('none')
            val_result = itm_eval(config, score_val_i2t, score_val_t2i, score_val_i2i, score_val_t2t,
                                val_loader, 
                                device=device, flag=False)
            val_result['text_neg'] = count_text_val
            val_result['image_neg'] = count_image_val
            print(do_45(val_result))
            test_result = itm_eval(config, score_test_i2t, score_test_t2i, score_test_i2i, score_test_t2t,
                                test_loader, 
                                device=device, flag=False)
            test_result['text_neg'] = count_text_test
            test_result['image_neg'] = count_image_val
            print(do_45(test_result))
            if 'coco' in config['data_path']:
                test_result_split = None
                for i in range(5):
                    tmp = itm_eval(config, score_test_i2t[i * 1000:(i + 1) * 1000, i * 5000:(i + 1) * 5000],
            ...
            if config['do_test_sl']:
                ...
                test_result = itm_eval(config, score_test_i2t, score_test_t2i, score_test_i2i, score_test_t2t,
                                    ...
                                    tau_t2i=1.0, tau_i2i=1.0, flag=True)
                test_result['text_neg'] = count_text_test
                test_result['image_neg'] = count_image_test
                print(do_45(test_result))
                if 'coco' in config['data_path']:
                    test_result_split = None
                    for i in range(5):
            ...
            result = val_result['r_mean']
            if config['do_neg_best']:
                result = (count_text_val + count_image_val) /2
            if result > best:
                save_obj = {
    ...
    torch.cuda.empty_cache()
    release_gpu_memory()
    return best_result, best, best_epoch
...
def train(model, train_loader, optimizer, lr_scheduler, epoch, warmup_steps, device, config, txt_enc_assisant, tokenizer=None):
    ...
    metric_logger.add_meter('loss_tri_neg_txt', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('loss_tri_neg_img', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    metric_logger.add_meter('loss_mv', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('up_loss', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('low_loss', utils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metrics = [
        "tau",
    ...
    print_freq = 50
    for i, (image, caption, image_features, raw_captions, idx, raw_captions_r, caption_neg, im_neg) in enumerate(train_loader):
        image = image.to(device, non_blocking=True)
        caption = caption.to(device, non_blocking=True)
        if config['do_neg']:
            im_neg = im_neg.to(device, non_blocking=True)
            caption_neg = caption_neg.to(device, non_blocking=True)
        ...
        # get loss
        cross_modal_loss, uni_modal_loss, contrastive_loss, tri_txt_neg_loss, tri_img_neg_loss, loss_mv, up_loss, low_loss, sl_loss = model(image, caption, image_features, caption_features, epoch, idx, caption_features_r, caption_neg, im_neg)
        
        loss = cross_modal_loss + uni_modal_loss + contrastive_loss + (tri_txt_neg_loss + tri_img_neg_loss) / 2 * config['tripletlossrate'] + loss_mv * 0.1
        ...
        metric_logger.update(loss_tri_neg_txt=tri_txt_neg_loss.item())
        metric_logger.update(loss_tri_neg_img=tri_img_neg_loss.item())
        metric_logger.update(loss_mv=loss_mv.item())
        metric_logger.update(up_loss=up_loss.item())
        ...
        if i % print_freq == 0 or i == len(train_loader) - 1:
            ...
            log = f'loss_tri_neg_txt: {tri_txt_neg_loss.item():.6f} ' + log
            log = f'loss_tri_neg_img: {tri_img_neg_loss.item():.6f} ' + log
            log = f'loss_mv: {loss_mv.item():.6f} ' + log
            log = f'up_loss: {up_loss.item():.6f} ' + log
            log = f'low_loss: {low_loss.item():.6f} ' + log
...
def release_gpu_memory():
    """
    彻底释放GPU显存（训练/验证每轮结束后调用）
    """
    # 1. 清空梯度（训练阶段必须，避免计算图残留）
    torch.cuda.empty_cache()
    
    # 2. 销毁未引用的张量（强制垃圾回收）
    gc.collect()
    
    # 3. （可选）清空CUDA缓存（释放PyTorch预留的未使用显存）
    torch.cuda.ipc_collect()
    
    # 4. （多卡/分布式场景）清空所有设备的缓存
    if torch.cuda.is_available():
...
def monitor_gpu_memory():
    """
    实时监控GPU显存，更新全局峰值变量
    需在训练/验证的前向/反向传播中调用（或注册钩子）
    """
    global gpu_mem_peak
    if torch.cuda.is_available():
        current_allocated = torch.cuda.memory_allocated()
        if current_allocated > gpu_mem_peak:
            gpu_mem_peak = current_allocated

def reset_gpu_peak_memory():
    """重置显存峰值统计（每轮开始前调用）"""
    global gpu_mem_peak
    gpu_mem_peak = 0
    torch.cuda.reset_peak_memory_stats()  # 重置PyTorch内置的峰值统计

def get_gpu_peak_memory(unit="GB"):
    """获取本轮显存峰值，支持GB/MB/Byte单位"""
    # 方式1：用PyTorch内置的峰值统计（更准确）
    peak_bytes = torch.cuda.max_memory_allocated()
    # 方式2：用自定义监控的峰值（兜底）
    if peak_bytes == 0:
        peak_bytes = gpu_mem_peak
...
if __name__ == '__main__':
    ...
    if args.fig is not None:
        fig_config = json.loads(args.fig)  # 转回字典
    ...
    itr = 0
    while True:
        gpu_info = get_gpu_remaining_memory(1)
        print('epoch: ', itr)
        ...
        itr += 1
        sys.stdout.flush()
        if gpu_info['free'] >= 22000:
            break
        time.sleep(60)
        
    config['abc_margin_alpha'] = 0.3
    config['tripletlossrate'] = 0.4
    if config['do_abcmargin_ab']:
        for i in range(10):
            config['abc_margin_alpha'] = i / 10
            main(args, config)
    elif config['do_tripletlossrate_ab']:
        for i in range(10):
            config['tripletlossrate'] = (i + 1) / 10
            main(args, config)
    else:
        main(args, config)
